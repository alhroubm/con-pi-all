...
def startDocker():
    ...
    if container.status == "created":
        if "motion" in requestedServices:
            ...
            logger.debug ("CONTAINER CREATE: SUCCESS")
            return 'Success'
        else:
            logger.debug ("CONTAINER CREATE: FAIL 1")
            return 'Fail'
    else:
        logger.debug ("CONTAINER CREATE: FAIL 2")
        return 'Fail'
...
def checkRemoteExecutions(IpAddress,ServicePort):
    ...
    logger.debug("Checked remote execution on " + str(IpAddress)+":"+str(ServicePort)+" status="+str(isRemoteExecutionWorking))
    ...
    for host, status in hosts_list:
        if status=='up' and host!=gateway and host!=ip:
            ...
            try:
                ...
                i=0 # just to skip the exception
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if offload == None or offload=='0': #offload is not set or set as False 
            ...
            logger.debug("offload is set to OFFLOAD")
            localBatteryLevel=getBatteryOfPIs(externalIP,servicePort)
            logger.debug("localBatteryLevel = " + str(localBatteryLevel))
            if localBatteryLevel != None: # Service is responding
                if int(localBatteryLevel) > 50: # Battery is high
                    ...
                    logger.debug("battery level low")
                    if remoteDockerStatus==0: # No remote docker, want to create
                        neighbourPIs = checkNeighbours() 
                        logger.debug("PRINTING ACTIVE NEIGHBOURS")
                        for PI in neighbourPIs:
                            logger.debug("-->" + PI)
                            remoteBatteryLevel = getBatteryOfPIs(PI,servicePort)
                            logger.debug("remoteBatteryLevel = " + str(remoteBatteryLevel))

                            # mo - this is where the selection algorithm takes place
                            # here it is simply dependent on batter level only and is quiet simplistic
                            # an improvement is where we take into account a bunch of other factors
                            # For example, there is no accounting for say distance from remote and 
                            # if for example historically the needed processing is small enough for the
                            # remote Pi to execute with endangering battery
