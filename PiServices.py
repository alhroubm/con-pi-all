...
def batteryFunc():
    global logger
    logger.debug('Battery: service requested')
## mo    pijuice = PiJuice(1,0x14)
## mo    charge = pijuice.status.GetChargeLevel()
## mo    return str(charge['data'])
    
    batterylevel = os.getenv('BATTERY', 30)
    if(batterylevel != None):
        charge = math.ceil(int(batterylevel))
        logger.debug("Battery level randomly set to " + str(charge))
    else:
        charge = int(batterylevel)
        logger.debug("Battery level randomly set from env var " + str(charge))
    return str(charge)

@app.route('/check')
def remoteCheckFunc():
    global addressSet
    logger.debug('check: addressSet check requested')
    global lock
    with lock:
...
def cameraFunc():
    global logger
    logger.debug('Camera: service requested')
    global lock
    with lock:
        camera = PiCamera()
...
def gpioAcFunc():
    ...
    logger.debug('Actuation: service requested')
    content = request.json
    ## mo GPIO.setup(int(content['repeller']), GPIO.OUT)
    ## mo time.sleep(15)
    ## mo GPIO.setup(int(content['repeller']), GPIO.IN)

    logger.debug ("mo: turn on repeller for 15 seconds")
    
    time.sleep(15)
    logger.debug ("mo: turn off repeller")
    return 'success'
...
def subscribeFunc():
    ...
    logger.debug('Subscribe: request from '+str(address))
...
def unsubscribeFunc():
    ...
    logger.debug('Unsubscribe: request from '+str(address))
